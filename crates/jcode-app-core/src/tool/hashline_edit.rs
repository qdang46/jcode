//! Surgical file edits anchored by SHA-256 line hash verification.
//!
//! The agent captures a hash of a line range during planning, then verifies
//! the hash before editing. If the file changed between plan and execution,
//! the edit is rejected with "anchor drifted" — fail closed.
//!
//! Delegates to the `hashline` crate for anchor semantics.
...
impl Tool for HashlineEditTool {
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        // Step 3: Write back atomically via temp file + rename.
...
mod tests {
    ...
    fn test_apply_edit_not_in_window() {
        let content = test_content();
        let result = apply_edit_within_window(content, 5,
            "    println!(\"hello\");",
            "    println!(\"world\");", 0);
        assert!(result.is_err());
    }
    ...
    fn test_multibyte_content() {
        ...
        let h = hash_window(content, 2, 3);
        assert!(!h.is_empty());
        let (_, start, _end) = apply_edit_within_window(content, 2,
            "    println!(\"你好\");",
            "    println!(\"hola\");", 0).unwrap();
        assert_eq!(start, 2);
    }
    ...
    fn test_apply_edit_on_last_line() {
        let content = "first\nsecond\nlast\n";
        let (new_content, start, end) = apply_edit_within_window(content, 3,
            "last", "final", 0).expect("editing the last line must work");
    ...
    fn test_apply_edit_on_only_line() {
        let content = "only\n";
        let (new_content, start, end) = apply_edit_within_window(content, 1,
            "only", "changed", 0).expect("single-line file must be editable");
    ...
    fn test_verify_anchor_and_apply_edit_use_consistent_window() {
        ...
        for anchor in 1usize..=5 {
            for ctx in 0usize..=2 {
                ...
                assert!(v.is_ok(), "verify failed for anchor={anchor}, ctx={ctx}, hash={h}: {v:?}");
            }
        }
    }
}
